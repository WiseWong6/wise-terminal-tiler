...
**Mixed Preview** — 实时混排编辑器，支持 Markdown / JSON / HTML / Mermaid 在同一个编辑器中混合编写并即时预览。纯客户端 SPA，无后端。
...
1. **Editor** 输入 → `code` state → **600ms debounce** (`useDebounce` hook) → `debouncedCode` → **MixedPreview**
...
| `components/MixedPreview.tsx` | ~2247 | 核心渲染引擎（最大的文件，包含多个内置子组件） |
...
- **内容类型检测**：`detectContentType()` 在 `MixedPreview.tsx` 中 — 通过首字符判定 JSON/Mermaid/HTML/Mixed/Markdown
